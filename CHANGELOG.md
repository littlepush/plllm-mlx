...
### Added
- `--chat-file` / `-f` parameter for batch prompts from file
- Process prompts sequentially with conversation history
- Auto-exit after last prompt

### Fixed
- Fix thinking_step_processor infinite loop when thinking ends
- Add `thinking_ended` flag to distinguish "not started" from "thinking ended"
- Use prompt_toolkit for better Chinese input support in chat

## [1.5.4] - 2025-03-17

### Fixed
- Display reasoning immediately when received, instead of waiting for content
- Better streaming experience for thinking models

## [1.5.3] - 2025-03-17

### Fixed
- Preserve trailing newline in prompt after KV cache split
- Fix model outputting extra `\n` at start due to missing trailing newline
- Use empty string join instead of newline join for gen_prompt construction
- Fix chat display format: `[You]`, `[Assistant]`, `[Reasoning]`
- Filter empty reasoning content in chat output

## [1.5.2] - 2025-03-16

### Fixed
- Correct thinking_step_processor logic based on old qwen3 implementation
- Use dynamic special_tokens instead of hardcoded tokens
- Properly detect thinking mode using begin_tokens and think_start_token
